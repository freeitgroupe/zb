...
/*
 * Формирование id для поставщика
 */
if (array_key_exists('pages_alias', $match)) {
    $alias = $match['pages_alias'];
    //поиск алиаса для поставщика
    $dataSuppliers = findOneParam('suppliers', 'alias', $alias);
    if(!$dataSuppliers){ header("Location:".PATH); exit; }
    if($dataSuppliers){
        $id = (int)$dataSuppliers['supplier_id'];
        $supplierId = (int)$dataSuppliers['supplier_id'];
    }
}else {
...
$title_header .= $dataSuppliers['title'];

//получаем массив с id продуктов для данного поставщика
$getArrProductsSupplier = getArrProductsSupplier($supplierId);
/*
...
if(isset($_POST['num_page_product'])){
    ...
    if(!$getArrProductsSupplier){exit('false');}
    $page_product_response = page_data_response($query="SELECT * FROM products WHERE id IN(" .implode(',',$getArrProductsSupplier) .") LIMIT {$num_page_product}, ".PERPAGE." ");
...
if(!$_POST)
    ...
    $breadcrumbs .= $dataSuppliers['title'] ;

/*=========Пагинация==========*/

// кол-во товаров на страницу
$perpage = (isset($_COOKIE['per_page']) && (int)$_COOKIE['per_page']) ? $_COOKIE['per_page'] : PERPAGE;// общее кол-во товаров
$count_goods = countProductsForSupplier($supplierId);
...
//продукты
//$products = Model_Driver::get_instance()->sqlQuery(['*'], '`products`',['`mark_view`'=>'1', '`stock_id`'=>$id], '`title`', 'ASC', "{$start_pos}, {$perpage}");
$products = Model_Driver::get_instance()->sqlQuery("SELECT * FROM products WHERE id IN(" .implode(',',$getArrProductsSupplier) .") LIMIT $start_pos, $perpage ");
