...
<?php // require_once 'top_nav_products.php'?>
        ...
        <form action="<?=$_SERVER['REQUEST_URI']?>" method="post" enctype="multipart/form-data">
            ...
            <div class="fist clear">
                <div class="boxOneThird">
                    <h3>Main</h3>
                    <input class="bottom" type="text" placeholder="Discount name*" name="title" value="<?=$discountInfo['title']?>">
                    <input type="text" data-range="true" data-multiple-dates-separator="/" class="datepicker-here bottom" value="" name="rangeTime" placeholder="">
                    <input class="bottom" type="text" placeholder="percent*" name="percent" value="<?=$discountInfo['percent']?>">
                    <h3>Activation</h3>
    ...
    <script>
        ...
        datepicker.selectDate([new Date('<?=$discountInfo['start_date'] ?>'), new Date('<?=$discountInfo['end_date'] ?>')]);
    </script>
<?php /* ?>
    <div class="contentBox clear">
        ...
        <div class="tableBox">
            <table>
                ...
                <?php if($discountInfo):?>
                    <?php foreach ($discountInfo as $item):?>
                        <tr>
                            <td>#<?=$item['id']?></td>
                            <td><?=$item['title']?></td>
