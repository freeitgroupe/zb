    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li><a href="/admin/news-options/">Новости</a></li>
                                ...
                                <form action="<?=$_SERVER['REQUEST_URI']?>" method="post" enctype="multipart/form-data" id="form_edit_pages">
                                    <?php for($i=0, $count=count($form_edit); $i < $count; $i++):?>
                                        ...
                                        <div class="col-md-12 form-group">
                                            ...
                                            <?php if($form_edit[$i]['type']=='select_category'):?>
                                                <select
                                                    name="<?=$form_edit[$i]['name_input']?>"
                                                    id="<?=$form_edit[$i]['name_input']?>"
                                                >
                                                    <?php for($s=0, $count_s=count($form_edit[$i]['option_value']); $s < $count_s; $s++):?>
                                                        <option
                                                            value="<?=$form_edit[$i]['option_value'][$s]['id']?>"
                                                            <?php if($form_edit[$i]['value_selected'] == $form_edit[$i]['option_value'][$s]['id']) echo 'selected'?>
                                                        >
                                                            <?=$form_edit[$i]['option_value'][$s]['title']?>
                                                        </option>
                                                    <?php endfor; ?>
                                                </select>
                                            <?php endif;?>
